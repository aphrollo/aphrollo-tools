...
| Its own test suite | 5 | 1477 test functions in `internal/tdd`, 320 `t.Parallel()` calls across 70 files, 927 s wall clock for the package. Windows CI does run it, in the `gate-env` job (`pipeline.yml:436`, `go test ./internal/tdd/... -count=1 -timeout=900s`), though it stays out of the `test-windows` allow-list and is gated on a code-change output. The measured 927 s against that 900 s budget is the live risk: the package is one slow box away from timing out its own CI job. | `go test ./internal/tdd/ -count=1 -timeout=1800s` for wall clock; `grep -ho 't\.Parallel()' internal/tdd/*_test.go \| wc -l` for parallelism; read the windows jobs in `.github/workflows/pipeline.yml` |
