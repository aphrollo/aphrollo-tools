...
// issue per check. That is a prompt to look, not a verdict: the answer may be
// to fix the check, to narrow it, or to demote it from deny to warn.
...
// does not already have an open one, and returns how many it opened. A check
// re-flagged every week must not collect a new issue every week: the open
// issue IS the record, and it is found by looking for the check's name in the
// open titles.
