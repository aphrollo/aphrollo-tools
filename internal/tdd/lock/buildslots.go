...
func acquireBuildSlot(targetDir string, deadline time.Duration, cmd, cwd string) (BuildSlot, func(), bool) {
	slot, release, wait := waitForBuildSlot(targetDir, deadline, cmd, cwd, func() bool { return false })
...
// starts).
func waitForBuildSlot(targetDir string, deadline time.Duration, cmd, cwd string, superseded func() bool) (BuildSlot, func(), SlotWait) {
	start := time.Now()
