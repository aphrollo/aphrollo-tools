...
func scopeMutantsRun(j MutantsJob) (files []string, carried []MutantOutcome, now TreeState, skipped []CarrySkip) {
	lane, ok := changedPaths(j.RepoRoot, j.BaseSHA, j.Tip)
	if !ok {
		return nil, nil, TreeState{}, nil
	}
	...
	invocationVersion := mutantsInvocationVersion(j.Worktree, producerVersion)
	files = PlanDiffFiles(j.RepoRoot, lane, now, cached, producerVersion, invocationVersion)
	...
	// producerVersion and invocationVersion as above: a version bump that
	...
	// invocationVersion per issue #531).
	plan := PlanMutants(laneWants(cached, lane), now, cached, producerVersion, invocationVersion)
