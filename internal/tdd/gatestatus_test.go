...
// for issue #430: one deferred job, one held build slot and one idle slot,
// all printed in one report.
func TestFormatGateStatus_ReportsAllThreeSources(t *testing.T) {
	...
	out := FormatGateStatus(jobs, slots, nil, now)
...
func TestFormatGateStatus_NoDeferredJobsSaysSo(t *testing.T) {
	out := FormatGateStatus(nil, nil, nil, time.Now())
...
func TestFormatGateStatus_ReportsQueueWaiter(t *testing.T) {
	...
	out := FormatGateStatus(nil, slots, waiters, now)
