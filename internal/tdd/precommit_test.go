...
func TestPrecommit_Mechanical_ScopedToStagedGoPackages(t *testing.T) {
	...
	want := Runner{"go", []string{"test", "-race", "-count=1", "-shuffle=on", "-timeout=180s", "./internal/x"}, "", time.Time{}}
...
func TestPrecommit_FailFirst_StableWorktreeUnderStateDir(t *testing.T) {
	...
	for i := 0; i < 2; i++ {
		if _, conclusive, _, _, _ := failFirstViolated(root, []string{"widget_test.go"}, run); !conclusive {
