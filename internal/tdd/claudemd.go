...
func ClaudeMDBlock(shimDir string, undercover bool) string {
	...
	b.WriteString("- **An open point is an ISSUE, never a markdown follow-up:** `aphrollo gate issue\n")
	b.WriteString("  \"<title>\" --label <theme>` opens one against this repo's remote, labelled from the\n")
	b.WriteString("  list it declares (`issue-labels`), and prints the URL as its only output. A gate\n")
	b.WriteString("  MISS is `aphrollo gate escape record` instead — never park either in a document.\n")
