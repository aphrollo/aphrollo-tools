...
func ClaudeMDBlock(shimDir string, undercover bool) string {
	...
	b.WriteString("  budget and continues; its result arrives at the next hook). The only sanctioned manual runs:\n")
	b.WriteString("  a mutation proof, a deliberate soak, or ONE targeted `-p <crate> <filter>` after a TIMEOUT.\n")
	...
	b.WriteString("- **An open point is an ISSUE, never a markdown follow-up:** `aphrollo gate issue \"<title>\"\n")
	...
	b.WriteString("  `main` takes merges and nothing else: the hooks deny an edit there, and the git shim refuses\n")
	b.WriteString("  `checkout -b`/`switch -c`, a move off main, and a commit not concluding a merge. Work in a lane:\n")
	b.WriteString("  `git worktree add -b lane/<name> <parent>/.worktrees/<repo>/<name> main`; override with\n")
	b.WriteString("  `APHROLLO_PRIMARY_EDITS=1` or `/tdd primary-edits on`.\n")
