...
func symbolRemovedHits(law Law, base BaseReader, files []string, content map[string]string) ([]Hit, error) {
	...
	for _, rel := range basePaths {
		...
		data, err := base.Read(rel)
		if err != nil {
			return nil, fmt.Errorf("law %q: reading %s at base: %w", law.Name, rel, err)
		}
...
func symbolRemovedLawHits(law Law, base BaseReader, files []string, content map[string]string, res *Result) ([]Hit, error) {
	if base == nil {
		res.Notes = append(res.Notes, law.Name+": skipped, no base (pass --base <ref>)")
		return nil, nil
	}
	return symbolRemovedHits(law, base, files, content)
}
