...
// newestFileMTime returns the most recent modification time among every file
// in the worktree, skipping .git.
func newestFileMTime(wt string) (time.Time, bool) {
	...
	err := filepath.WalkDir(wt, func(path string, d fs.DirEntry, err error) error {
		...
		// .git is a directory in a normal clone but a plain FILE (a "gitdir:
		// ..." pointer) in a linked worktree — skip it either way; a linked
		// worktree rewrites that pointer on every checkout, which would
		// otherwise read back as "just touched" regardless of the actual
		// tracked files' age.
		if d.Name() == ".git" {
			if d.IsDir() {
