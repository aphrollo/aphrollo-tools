...
| `gate output` | manual | Read-only: prints the TEXT of the last settled suite run the gate itself made for this repo root — a header (when, which stage, which root, the directory the run executed in, which command, which verdict, how long, and whether the stored bytes were truncated) followed by the run's own output, byte for byte. `gate stats` answers what the verdict WAS; this answers what the run PRINTED, so reading one assertion line costs no re-run — which is what the narrowed-rerun refusal now points at. One record per root, overwritten by the next settled run and capped at 256 KB (the TAIL is kept, since a failure prints at the end). Exits non-zero, saying which, when no run is recorded for this root or the record is older than the 30-minute freshness window. |
...
| 6 | suites of the touched crates + every crate downstream of them | full build + link + run | the heaviest, and therefore last. The merge runs it; the commit names it on a `NOT RUN` line. Downstream is the same list stage 4's check scope prints: a crate that depends on a touched one is where that change's effect is pinned, so a numerical change is judged where the pin lives. With no dependents it is the touched crates alone, never `--workspace` |
| 7 | `cargo test -p <crate> --doc` over stage 6's crates | one rustdoc run per crate that has a doc fence | **nextest does not run doctests at all**, so a `compile_fail` proof — the only way to assert something must NOT compile — would never execute. Scoped by a grep of the crate's `src/`: a crate with no doc fence buys no run |
...
`prove` holds, mutates, verifies that the write landed (the content git reads
for the file hashes differently after the write than from the bytes the proof
started with, so unstaged work already in the file and an untracked file are
both judged on the mutation alone), runs the file's related tests scoped to
the `--want-fail` test with fail-fast off, and restores the file byte-identically —
